...
# an alternative exists, as an attribute that includes gene update number is available (need to add)
...
# 5.2.1. Remove gene version - same as above
...
# --- Filter your results - establish your padj and L2FC cutoffs
# --- using tidyverse
res_tb <- res_sal_shrink %>%
  +     data.frame() %>%
  +     rownames_to_column(var="gene") %>% 
  +     as_tibble()

res_tb <- res_tb %>% 
  +     mutate(threshold_OE = padj < 0.05 & abs(log2FoldChange) >= 1.5)

# 6.2. Create volcano plot
vol_sal1 <- ggplot(res_tb) +
