...
DSal <- merge(DSal, `MDAMB231-S`[c("Name", "NumReads")], by = "Name")
...
# 1.2. DESeq2 dataset generation (may need to convert characters to factors)
...
# An alternative to name is contrast = c("Condicion", "T", "NT")
...
merge(res_sal_df, trans.ids, by = "transannot")

# 4.0. Subsetting DE genes by significance and L2FC
bc05 <- subset(res_sal_df, res_sal_df$padj < 0.01)
bc_cut <- subset(bc05, bc05$log2FoldChange > 2.0)
...
# 5.0. vsd transformation for heatmap & PCA generation
...
mat_sal <- assay(vsd_sal)[topVarTranscripts, ]
mat_sal <- mat_sal - rowMeans(mat_sal)
