...
txi <- tximport(files, type="salmon", tx2gene=t2g[,c("ensembl_transcript_id", "ensembl_gene_id")], 
...
bc_subtype <- log10(txi$abundance + 1)

# --- Assign column names to your samples (optional)
...
colnames(bc_annot) <- "ensembl_gene_id" # Optional, merge() can handle different colnames (specify by.x and by.y)

# --- Merge annotation file with gene_biotype (which contains c("ensembl_gene_id", "gene_biotype" & "entrez_gene_id")
# --- An object similar to gene_biotype should be previously obtained; I used biomaRt for annotation
...
table(ssp.subtypes$subtype, matrix$Subtipo) # --- A cross-table can be built in order to see how subtyping matches (or mismatches) your samples
#         NM HER2 LA LB TNBC
# Basal   5    3  0  0    9
# Her2    0    6  0  3    0
# LumB    0    1  8  2    0
